...
PRIMO = 2**23209-1 #número de Marsenne con n=23209 (todos los números
                # de Marsenne son primos)
...
def reconstruccion(partes):
    ...
    for j, secreto_de_j in enumerate(partes):
        xj, yj = secreto_de_j
        productorio = Fraction(1,1)

        for m, secreto_de_m in enumerate(partes):
            xm, ym = secreto_de_m
            if m != j:
                productorio *= Fraction(Fraction(xm, 1), Fraction(xm - xj, 1))

        productorio *= yj
        sumatorio += Fraction(productorio, 1)

    return int(round(Fraction(sumatorio, 1), 0))
...
if __name__ == '__main__':
    ...
    if (s > PRIMO):
        raise Exception ('Tu secreto es demasiado largo')
    ...
    # Para comprobar que solo con n personas es suficiente para reconstruir
    # el secreto, pasamos a nuestro método de Lagrange solo n puntos

    sol_n = sol[0:n+1]
    recon=str(reconstruccion(sol_n))
